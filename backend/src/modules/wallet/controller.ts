...
walletRouter.post('/transfer', async (req: AuthRequest, res) => {
  ...
  try {
    ...
    // Bug real corregido: la transferencia se hacía bien (atómica, sin doble
    // gasto), pero quien RECIBÍA la plata nunca se enteraba — ni push, ni
    // aviso en vivo. Mismo patrón ya corregido esta sesión para mensajes,
    // stickers y logros: un sistema que hace su trabajo pero no avisa nada
    // es, para quien lo usa, indistinguible de que no pasó nada.
    const sender = await prisma.user.findUnique({ where: { id: req.userId! }, select: { name: true } });
    io.to(`user:${toUserId}`).emit('wallet_received', { fromUserId: req.userId, amount, note });
    sendPushNotification(toUserId, 'Recibiste un pago 💸', `${sender?.name || 'Alguien'} te envió $${amount}`, req.userId!)
      .catch((e) => logger.error('Error enviando push de transferencia', e));
