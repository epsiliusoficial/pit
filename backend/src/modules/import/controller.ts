...
importRouter.post('/whatsapp', upload.single('file'), async (req: AuthRequest, res) => {
  ...
  // Bug real corregido: importar hasta 5000 mensajes no avisaba a NADIE —
  // otro miembro mirando ese chat en ese momento no se enteraba de nada
  // hasta recargar manualmente. No se emite un 'new_message' por cada línea
  // (inundaría la sala con miles de eventos); un solo evento le dice al
  // cliente que hay historial nuevo para refrescar.
  io.to(chatId).emit('history_imported', { chatId, count: created.length });
