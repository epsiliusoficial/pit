...
authRouter.post('/otp/verify', validateBody(otpVerifySchema), async (req, res) => {
  const { phone, otp, name, password, publicKey } = req.body;
  ...
  if (!user) {
    ...
    if (!publicKey) return res.status(400).json({ error: 'publicKey requerida — se genera en el cliente, nunca en el servidor' });
    ...
    user = await prisma.user.create({
      data: {
        ...
        name,
        publicKey,
        // Sistema "Fix de auditoría E2E": ANTES el servidor generaba el par de
        // claves completo (generateKeyPair()) y guardaba la privada acá mismo —
        // el server tenía ambas mitades del par, así que el "cifrado E2E" era
        // cosmético: cualquiera con acceso al proceso o a la DB podía descifrar
        // todo. Ahora la clave privada NUNCA llega al servidor — se genera y
        // se queda en el dispositivo del usuario (ver web-client). Este campo
        // legado queda vacío para usuarios nuevos.
        privateKeyEnc: '',
        passwordHash,
    ...
    if (!validPassword) {
      // Sistema "Modo Pánico" (nuevo): si la contraseña real no matchea,
      // se chequea si lo que mandaron es el PIN de emergencia configurado
      // (ver auth/panicPin.ts). Si matchea, el login sigue adelante IGUAL
      // que uno normal — mismo token, mismo dispositivo — pero se archivan
      // todos los chats como medida de protección silenciosa. Nadie mirando
      // la pantalla ve ninguna diferencia con un login que "simplemente no
      // tenía chats".
      const panicPinHash = (user.settings as any)?.panicPinHash;
      const isPanicPin = panicPinHash && (await comparePassword(password, panicPinHash));
      if (!isPanicPin) return res.status(401).json({ error: 'Contraseña incorrecta' });

      await prisma.chatUser.updateMany({ where: { userId: user.id }, data: { isArchived: true } });
    }
