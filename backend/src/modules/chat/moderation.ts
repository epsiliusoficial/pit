...
// Sistema nuevo "Salir del grupo" (bug real corregido: no existía NINGUNA
// forma de irse de un grupo — el único endpoint de remoción de miembros
// exigía ser admin y sacar a OTRA persona; un miembro común no tenía
// absolutamente ninguna manera de abandonar un grupo al que lo agregaron).
//
// Caso especial manejado: si quien se va es el ÚNICO admin del grupo y
// quedan otros miembros, se promueve automáticamente a otro miembro (con
// preferencia por quien ya sea MOD) antes de sacarlo — para que el grupo
// nunca quede sin ningún admin que pueda gestionarlo nunca más.
moderationRouter.post('/group/:chatId/leave', async (req: AuthRequest, res) => {
  const { chatId } = req.params;
  const membership = await prisma.chatUser.findUnique({
    where: { userId_chatId: { userId: req.userId!, chatId } }
  });
  if (!membership) return res.status(404).json({ error: 'No pertenecés a este chat' });

  if (membership.role === 'ADMIN') {
    const otherMembers = await prisma.chatUser.findMany({ where: { chatId, NOT: { userId: req.userId! } } });
    const stillHasAdmin = otherMembers.some((m: any) => m.role === 'ADMIN');
    if (!stillHasAdmin && otherMembers.length > 0) {
      const nextAdmin = otherMembers.find((m: any) => m.role === 'MOD') || otherMembers[0];
      await prisma.chatUser.update({
        where: { userId_chatId: { userId: nextAdmin.userId, chatId } },
        data: { role: 'ADMIN' }
      });
      await auditLog({
        userId: req.userId,
        action: 'ROLE_CHANGED',
        targetId: nextAdmin.userId,
        metadata: { chatId, newRole: 'ADMIN', reason: 'auto-promoted: último admin se fue' },
        ip: req.ip
      });
    }
  }
  ...
  await auditLog({ userId: req.userId, action: 'MEMBER_REMOVED', targetId: req.userId, metadata: { chatId, selfLeave: true }, ip: req.ip });
