...
chatListRouter.get('/', async (req: AuthRequest, res) => {
  ...
  const result = memberships.map((m: any) => {
    ...
    return {
      ...
      // decryptContent es seguro para ambos formatos: si es un sobre E2E (no
      // empieza con "enc1:") lo devuelve tal cual; si es contenido viejo
      // cifrado por el servidor (auto-respuestas, etc.) lo descifra de verdad.
